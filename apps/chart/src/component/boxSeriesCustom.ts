...
export default class BoxSeriesCustom extends Component {
  ...
  render<T extends BarChartOptions | ColumnChartOptions | ColumnLineChartOptions>(
    ...
    const renderOptions: RenderOptions = {
      ...
      widthPerSeries: options.series!.widthPerSeries!,
    };
  ...
  renderSeriesModel(
    ...
    const { tickDistance, diverging, widthPerSeries } = renderOptions;
    ...
    seriesData.forEach(({ data, color: seriesColor, name, colorByCategories }, seriesIndex) => {
      const columnWidth = widthPerSeries![seriesIndex];
      const padding = (tickDistance - columnWidth) / 2;
